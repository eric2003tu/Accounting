...
function IncomePageContent() {
  ...
  const handleBusinessChange = (businessId: number) => {
    ...
    router.replace(`/manager/income?businessId=${businessId}`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add Income Button */}
      <Link
        href={`/manager/income/add?businessId=${selectedBusiness.id}`}
...
export default function IncomePage() {
  return (
    <Suspense fallback={<BrandLoadingScreen title="Loading income" subtitle="Preparing income records for your assigned business." />}>
      <IncomePageContent />
    </Suspense>
  );
}
